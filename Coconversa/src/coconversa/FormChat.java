...
//        panelListaGruposChat.add(new JLabel(Grupo+"   "));
    ...
    public void actionPerformed(ActionEvent ae) {
       ...
       {           
            ...
            FormCrearGrupo cGrupo = new FormCrearGrupo(OOS, Usuario, ListUsuario);
       ...
       if(ae.getSource()==btnMensajeGruposChat){
           if(listaGruposChat.getSelectedValue()!=null){  
               ...
               for (int i = 0; i < ChatsAbiertos.size(); i++) {
                   if (ChatsAbiertos.get(i).nombre.equals(listaGruposChat.getSelectedValue())) {
                       creado = true;
                   } else creado = false;
               }
               if (!creado) {
                    chat michat = new chat(listaGruposChat.getSelectedValue().toString(),true);
                     ...
                     pedirmensajes.getMensajesGrupo(michat.nombre);
               }
           }else{
         ...
         if(ae.getSource() == btnMensajeAmigoChat){
           if(listaAmigosConectadosChat.getSelectedValue()!=null){
               boolean creado = false;
               for (int i = 0; i < ChatsAbiertos.size(); i++) {
                   if (ChatsAbiertos.get(i).nombre.equals(listaAmigosConectadosChat.getSelectedValue())) {
                       creado = true;
                   } else creado = false;
               }
               if (!creado) {
        ...
        /*txtMensajes.addKeyListener(new KeyListener(){
            ...
            public void keyPressed(KeyEvent ke) {
               ...
               else
               {
                   ...
                   {
                       txtMensajes.setText(txtMensajes.getText().substring(0,txtMensajes.getText().length()));
                   }
                   else
                   {
                       txtMensajes.setText(txtMensajes.getText()+ke.getKeyChar());
                   }
               }
            }
